...
export default function AddProject() {
  ...
  const resetForm = () => {
    ...
    setStatus('قيد التنفيذ'); setWorkers(0); setTechs(0);
    setImageFile(null);
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: insertError } = await supabase
        .from('projects')
        .insert([{ 
          project_name: name, location: location, completion_rate: rate, 
          status: status, workers_count: status === 'قيد التنفيذ' ? workers : 0,
          techs_count: status === 'قيد التنفيذ' ? techs : 0,
          last_updated: updateDate, image_url: publicImageUrl
        }]);
      ...
      <form onSubmit={handleAdd} className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-6 rounded-2xl border border-slate-200">
          <div className="md:col-span-2">
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} required placeholder="مثلاً: فيلا منطقة السرة" className="w-full border-2 border-slate-300 p-3 rounded-xl focus:border-blue-500 outline-none transition-all bg-white text-slate-800 font-bold" />
          </div>
          <div>
        ...
        <div className="bg-white p-6 rounded-2xl border-2 border-blue-50 shadow-sm">
          ...
          {/* تم إجبار هذا الجزء على LTR عشان السحب واللون يمشوا صح مع الأرقام */}
          ...
          {status === 'قيد التنفيذ' && (
            <>
              <div className="bg-orange-50 p-3 rounded-xl border border-orange-100">
                ...
                <input type="number" value={workers} onChange={(e) => setWorkers(parseInt(e.target.value))} className="w-full border-2 border-orange-200 p-2 rounded-lg outline-none text-slate-800 font-bold" />
              </div>
              <div className="bg-blue-50 p-3 rounded-xl border border-blue-100">
                <label className="block text-blue-700 font-bold mb-2 text-sm">عدد الفنيين 👨‍🔧</label>
                <input type="number" value={techs} onChange={(e) => setTechs(parseInt(e.target.value))} className="w-full border-2 border-blue-200 p-2 rounded-lg outline-none text-slate-800 font-bold" />
              </div>
            </>
          )}
        </div>
        ...
        <div className={`p-6 rounded-2xl border-4 border-dashed transition-all text-center ${imageFile ? 'border-green-400 bg-green-50' : 'border-slate-300 bg-slate-50 hover:border-blue-400'}`}>
          ...
          <label htmlFor="file-upload" className="cursor-pointer bg-blue-600 text-white px-8 py-3 rounded-full font-bold hover:bg-blue-700 shadow-md inline-block">
        ...
        <button 
          ...
          className={`w-full py-5 rounded-2xl font-black text-xl shadow-2xl transition-all active:scale-95 flex items-center justify-center gap-3 ${uploading ? 'bg-slate-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-8
