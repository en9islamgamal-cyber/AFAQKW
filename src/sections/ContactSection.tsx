...
const ContactSection = ({ className = '' }: ContactSectionProps) => {
  ...
  return (
    // التعديل: الخلفية بيضاء بدل الكحلي الغامق
      ...
      <div className="px-6 lg:px-[7vw]">
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-16">
          ...
          <div ref={formCardRef} className={`lg:w-[55%] ${isAr ? 'lg:pr-8 text-right' : 'lg:pl-8 text-left'}`}>
            {/* التعديل: خلفية الفورم بقت رمادي فاتح جداً (أبيض مكسور) مع Shadow */}
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      {/* التعديل: الـ Input بقى أبيض والخط كحلي */}
                      <Input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder={isAr ? 'مثال: محمد أحمد' : 'e.g. John Doe'} required className="h-14 bg-white border-gray-300 text-[#0F172A] rounded-xl focus:border-[#FF6A00] focus:ring-[#FF6A00] font-bold shadow-sm" />
                    </div>
                    <div className="space-y-2">
                      <label className="text-xs text-[#1F2937] font-black uppercase tracking-widest">{isAr ? 'البريد الإلكتروني' : 'Email'}</label>
                      <Input type="email" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} placeholder="email@afaq.com" required className="h-14 bg-white border-gray-300 text-[#0F172A] rounded-xl focus:border-[#FF6A00] focus:ring-[#FF6A00] font-bold shadow-sm" />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      <Input type="tel" value={formData.phone} onChange={(e) => setFormData({ ...formData, phone: e.target.value })} placeholder="+965 ---- ----" required className="h-14 bg-white border-gray-300 text-[#0F172A] rounded-xl focus:border-[#FF6A00] focus:ring-[#FF6A00] font-bold shadow-sm" />
                    </div>
                    <div className="space-y-2" dir={isAr ? 'rtl' : 'ltr'}>
                      <label className="text-xs text-[#1F2937] font-black uppercase tracking-widest">{isAr ? 'نوع الخدمة المطلوبة' : 'Service Type'}</label>
                      <Select value={formData.projectType} onValueChange={(value) => setFormData({ ...formData, projectType: value })}>
                        <SelectTrigger className="h-14 bg-white border-gray-300 text-[#0F172A] rounded-xl focus:border-[#FF6A00] focus:ring-[#FF6A00] font-bold shadow-sm">
                        ...
                        <SelectContent className="bg-white border-gray-200 text-[#0F172A] font-bold shadow-lg">
                          <SelectItem value="MEP" className="hover:bg-gray-100 focus:bg-gray-100">{isAr ? 'أنظمة كهروميكانيكية (MEP)' : 'MEP Systems'}</SelectItem>
                          <SelectItem value="Construction" className="hover:bg-gray-100 focus:bg-gray-100">{isAr ? 'مقاولات عامة' : 'General Construction'}</SelectItem>
                          <SelectItem value="Firefighting" className="hover:bg-gray-100 focus:bg-gray-100">{isAr ? 'أنظمة مكافحة الحريق' : 'Firefighting Systems'}</SelectItem>
                          <SelectItem value="Infrastructure" className="hover:bg-gray-100 focus:bg-gray-100">{isAr ? 'بنية تحتية' : 'Infrastructure'}</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-xs text-[#1F2937] font-black uppercase tracking-widest">{isAr ? 'اسم الشركة (اختياري)' : 'Company Name'}</label>
                    <Input type="text" value={formData.company} onChange={(e) => setFormData({ ...formData, company: e.target.value })} placeholder={isAr ? 'شركة...' : 'Company name...'} className="h-14 bg-white border-gray-300 text-[#0F172A] rounded-xl focus:border-[#FF6A00] focus:ring-[#FF6A00] font-bold shadow-sm" />
                  </div>

                  <div className="space-y-2">
                    <label className="text-xs text-[#1F2937] font-black uppercase tracking-widest">{isAr ? 'تفاصيل المشروع' : 'Project Details'}</label>
                    <Textarea value={formData.message} onChange={(e) => setFormData({ ...formData, message: e.target.value })} placeholder={isAr ? 'اكتب باختصار عن مشروعك...' : 'Briefly describe your project...'} rows={4} required className="bg-white border-gray-300 text-[#0F172A] rounded-2xl focus:border-[#FF6A00] focus:ring-[#FF6A00] font-bold resize-none shadow-sm" />
                  </div>

                  <button type="submit" disabled={isSubmitting} className="w-full py-5 bg-[#FF6A00] hover:bg-orange-600 text-white rounded-2xl font-black text-xl transition-all shadow-lg transform hover:-translate-y-1 flex items-center justify-center gap-3 disabled:opacity-50 disabled:hover:translate-y-0">
                    {isSubmitting ? (
                      <><div className="w-5 h-5 border-3 border-white/30 border-t-white rounded-full animate-spin" /> {isAr
